...
export const BotSharing = ({ children }: { children?: React.ReactNode }) => {
  ...
  const handleSharing = useCallback(async () => {
    if (!bot?.id) return;
    const res = await apiClient.defaultApi.botsBotIdSharingPost({
      botId: bot.id,
      botPublishRequest: {
        group_ids: selectedDepartents,
      },
    });
  }, [bot?.id, selectedDepartents]);
  ...
  useEffect(() => {
    if (sharingVisible && departments.length === 0) {
      getDepartments();
    }
  }, [departments.length, getDepartments, sharingVisible]);
    ...
    <Dialog open={sharingVisible} onOpenChange={() => setSharingVisible(false)}>
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={selectedDepartents.length === 0}
