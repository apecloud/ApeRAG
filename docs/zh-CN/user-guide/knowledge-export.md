...
若中途失败(网络 / 存储错误 / 用户取消),状态会变成 `FAILED`,对话框展示错误信息。**对话框需要保持打开**直到完成或失败 — 关闭页面会丢失当前任务进度引用,当前不支持关闭后恢复进度。
...
- 下载链接有效期:**7 天**后由系统自动清理
- 用户应在 7 天内完成下载;过期后需要重新触发导出

### 大知识库的预估

- 系统采用**流式打包**(边扫描边写 ZIP),不会在内存里堆积全部对象
- 大知识库(上万文档 / 上百 GB)的导出时间主要受存储吞吐限制
- 进度条里的 `已处理 / 总数` 比例可用于预估剩余时间
...
当前**不支持关闭后恢复进度**。关闭对话框等于放弃当前任务的进度引用;后台打包任务仍会继续,但前端无法恢复进度展示。
...
- 了解文档处理的后端架构:[模块化 current-state 架构](../../modularization/architecture.md) 的 `knowledge_base` / `indexing` domain sections
