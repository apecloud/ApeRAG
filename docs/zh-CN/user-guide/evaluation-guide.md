...
Evaluation 采用"两个核心对象 + 单入口"的工作流：

1. **Evaluation Dataset** — 一组用于评测的问答，挂在某个 Collection 下。
2. **Evaluation Run** — 对某个 Dataset 发起的一次运行；按用户的"默认 Bot"执行，必要时显式覆盖 `bot_id`。

**操作只有 3 步**：创建 Dataset → 录入问题 → 发起评测。

以下旧概念已经在 `#20` 里彻底移除，不再在 UI 和 API 上出现：

- ~~Benchmark~~
- ~~Dataset Version / Publish Version~~
- ~~Question Set（独立于 Dataset 的问题集）~~
- ~~发起运行时要手动输入 / 复制 `dataset_version_id`~~
- ~~发起运行时在 FE 页面手工选择 Bot~~（改为默认 Bot 解析 + 可覆盖）
...
- `/workspace/collections/{collectionId}/evaluations`
  ...
  - 手动录入问题（每条是一个 dataset item）。
  - 点"发起评测"启动一次 Evaluation Run。
  - 查看该 Collection 的历史 runs 和进度。

**Dataset 问题管理**（进入某个 dataset 后）：

- `/workspace/collections/{collectionId}/evaluations/datasets/{datasetId}`
  - 查看/新增/删除该 dataset 下的问题。
  - 注意：dataset item 可以修改，但**历史 run 通过 value-copy 快照保留当时的问答内容**，不会被后续编辑影响。
...
- 对失败 item 发起重试；run 处于 `queued` / `running` 时可以取消整条 run。

**Bot → Evaluation 页面**（只读历史列表）：

- `/workspace/bots/{botId}/evaluation`
  - 只读展示这个 bot 作为评测对象的历史 runs。
  - 不再是"发起运行"的入口，也没有 `dataset_version_id` / Bot 选择输入。
...
一条 run item = 一个 dataset item 的**快照 + 执行态**。**不回读 mutable dataset items**：`input_message` / `expected_answer` / `reference_context` 在创建 run 时 value-copy 到 run item，确保 dataset 后续编辑不影响历史 run 的语义。

### Evaluation Run Item Attempt

一次实际调用机器人的 attempt。挂在 run item 下，通过 `attempt_no` 编号；失败后可以重试，新的 attempt 以追加的形式加入。
...
- **Dataset 没有问题就点"发起评测"**：按钮置灰，hover 提示"请先给数据集添加至少一个问题"。
- **当前用户没有任何可用的 Bot** 就发起评测（既没有 `Default Agent Bot` 也没有其它 active bot）：FE 会把后端返回的 `ValidationException` 替换成用户可读的提示："当前没有可用于评测的 Bot，请先创建 Bot 或联系管理员。"
- **Dataset 被删除**：历史 run 通过 snapshot 保留；dataset 删除后仍可查看 run 的每条 item（显示 `dataset_name` 而不是挂 FK）。
- **非终态 run**（`queued` / `running`）：run detail 页每 5 秒自动刷新，直到 run 进入终态。
- **terminal 状态语义**：`COMPLETED` / `FAILED` / `CANCELLED` 三种是终态；进入终态后不再接收 `RUNNING` 状态切换（防止 worker 回写覆盖已取消的 run，详见 `architecture/conversation-agent-evaluation.md` 中 "`EvaluationRunStatus.is_terminal()`" 一节）。
...
所有 `evaluation_v2` 接口落在 evaluation domain（[`architecture/conversation-agent-evaluation.md`](../architecture/conversation-agent-evaluation.md#domain-evaluation)），`/api/v2` 前缀。旧路径 `/api/v2/benchmark-datasets*`、`/api/v2/benchmark-datasets/{id}/versions*` 以及请求/响应字段 `dataset_version_id` 都已移除。
