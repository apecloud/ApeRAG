"""
Index Reconciler - Backend Chain

This module handles the backend chain: scanning document index specs and updating statuses.
It's called by the periodic Celery task to reconcile actual index state with desired state.

Simple design:
- Scan all specs where status.observed_version < spec.version OR state mismatch
- For each mismatch, dispatch async task to create/update/delete index
- Update status.observed_version when reconciliation starts
"""
...
# Global instance
index_reconciler = IndexReconciler()
