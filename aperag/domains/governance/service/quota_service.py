...
Phase 8 #66 moves the implementation under the governance domain. The
legacy ``aperag.service.quota_service`` module remains only as a
compatibility shim for existing Protocol/DI consumers.
"""
...
class QuotaService:
    ...
    async def get_all_users_quotas(self, search_term: str = None) -> List[Dict]:
        ...
        async def _query(session):
            ...
            user_table = Table(USER_TABLE, Base.metadata, autoload_with=session.bind)
            ...
            stmt = select(
                ...
                user_table.c.email,
                user_table.c.role,
            ).where(user_table.c.gmt_deleted.is_(None))
            ...
            if search_term and search_term.strip():
                ...
                stmt = stmt.where(
                    or_(
                        user_table.c.username == search_value,
                        user_table.c.email == search_value,
                        user_table.c.id == search_value,
                    )
                )
