...
Phase 4 Step 4-S4: moved from ``aperag.service.marketplace_service``
to the ``aperag.domains.marketplace.service`` canonical location.
G1 boundary pass: imports rewritten to domain paths.
Legacy shim at the original path forwards for back-compat.
"""
...
class MarketplaceService:
    ...
    async def publish_collection(self, user_id: str, collection_id: str) -> None:
        ...
        await self.db_ops.create_or_update_collection_marketplace(
            collection_id=collection_id, status=CollectionMarketplaceStatusEnum.PUBLISHED.value
        )
    ...
    async def get_sharing_status(self, user_id: str, collection_id: str) -> Tuple[bool, Optional[datetime]]:
        ...
        is_published = marketplace.status == CollectionMarketplaceStatusEnum.PUBLISHED.value
    ...
    async def validate_marketplace_collection(self, collection_id: str):
        ...
        if not marketplace_record or marketplace_record.status != CollectionMarketplaceStatusEnum.PUBLISHED.value:
    ...
    async def list_published_collections(
        ...
        return SharedCollectionList(items=collections, total=total, page=page, page_size=page_size)
    ...
    async def _verify_collection_ownership(self, user_id: str, collection_id: str) -> Collection:
