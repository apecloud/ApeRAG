...
class LLMTxtSearchProvider(BaseSearchProvider):
    ...
    async def search(
        ...
        Args:
            query: Search query (not used in LLM.txt discovery)
            max_results: Maximum number of results to return
            search_engine: Search engine to use
            timeout: Request timeout in seconds
            locale: Browser locale (not used in LLM.txt discovery)
            source: Domain or URL to search for LLM.txt files
            use_source_domain_only: Not used in this provider

        Returns:
            List of search result items from LLM.txt files
        """
        # Validate parameters
        if max_results <= 0:
    ...
    async def _discover_llms_txt_for_domain(self, domain: str, timeout: int = 30) -> Optional[WebSearchResultItem]:
        ...
        for pattern in self.LLM_TXT_PATTERNS:
            ...
            try:
                ...
                if (read_response.results and 
                    read_response.results[0].status == "success" and 
                    read_response.results[0].content):
    ...
    async def _discover_llms_txt_for_domains(self, domains: List[str], timeout: int = 30) -> List[WebSearchResultItem]:
        ...
        tasks = [
            self._discover_llms_txt_for_domain(domain, timeout)
