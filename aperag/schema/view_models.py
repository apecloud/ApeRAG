...
#   timestamp: 2025-07-11T09:52:54+00:00
...
class WebSearchRequest(BaseModel):
    ...
    source: Optional[str] = Field(
        None,
        description="Domain or URL for targeted processing. Can be a domain name (e.g., 'vercel.com') or URL (e.g., 'https://vercel.com/docs/llms.txt'). When provided, the system will discover LLM-optimized content (llms.txt) and perform site-specific searches.",
        example='vercel.com',
    )
    use_source_domain_only: Optional[bool] = Field(
        False,
        description="Strict domain limitation flag. When true, only returns results from the domain specified in 'source'. When false (default), may include additional relevant results from other domains. Has no effect when 'source' is empty.",
        example=False,
    )
    search_llms_txt: Optional[bool] = Field(
        True,
        description="Enable LLM.txt discovery search. When true, searches for LLM-optimized content indexes (llms.txt files) from the specified domain in 'source'. When 'source' is not provided, this feature is silently skipped. LLM.txt files provide AI-optimized content summaries and documentation.",
        example=True,
    )
