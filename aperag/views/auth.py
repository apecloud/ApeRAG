...
async def test_register_admin(
    request: Request, data: view_models.Register, session: AsyncSessionDep, x_test_secret: str = Header(None)
):
    if os.environ.get("TEST_ENV") != "1":
        raise HTTPException(status_code=403, detail="Not allowed")

    secret = os.environ.get("TEST_ADMIN_SECRET", "").strip()
    if not secret or secret != x_test_secret:
        raise HTTPException(status_code=403, detail="Not allowed")
    ...
    user = User(
        username=data.username,
        email=data.email,
        hashed_password=UserManager(None).password_helper.hash(data.password),
        role=Role.ADMIN,
        is_active=True,
        is_verified=True,
        date_joined=utc_now(),
