...
async def get_chat_view(
    request: Request, bot_id: str, chat_id: str, user: User = Depends(get_current_user_with_state)
) -> view_models.Chat:
...
@router.get("/bots")
async def list_bots_view(request: Request, user: User = Depends(get_current_user_with_state)) -> view_models.BotList:
...
async def delete_model_service_provider_view(
    request: Request, provider: str, user: User = Depends(get_current_user_with_state)
):
