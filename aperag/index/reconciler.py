...
class BackendIndexReconciler:
    ...
    def _get_and_lock_indexes_needing_reconciliation(self, session: Session, document_ids: List[str] = None) -> List[DocumentIndex]:
        """
    ...
    def _claim_indexes_for_creating(self, session: Session, document_ids: List[str] = None) -> List[DocumentIndex]:
        ...
        update_stmt = (
            ...
            .values(
                actual_state=IndexActualState.CREATING,
    ...
    def _claim_indexes_for_deleting(self, session: Session, document_ids: List[str] = None) -> List[DocumentIndex]:
        ...
        update_stmt = (
            ...
            .where(and_(*conditions))
            .values(
                actual_state=IndexActualState.DELETING,
                gmt_updated=func.now(),
                gmt_last_reconciled=func.now()
            )
            .returning(DocumentIndex)
    ...
    def _reconcile_grouped(self, session: Session, indexes_needing_reconciliation):
        ...
        for doc_index in indexes_needing_reconciliation:
            ...
            # Determine operation type based on desired state
            if doc_index.desired_state == IndexDesiredState.PRESENT:
                if doc_index.actual_state == IndexActualState.CREATING:
                    # Check if this was a retry (from FAILED state) or initial creation
                    operation_type = "create"  # Both initial and retry use create
        ...
        for operation_type, doc_indexes in operations.items():
            for doc_index in doc_indexes:
                try:
                    ...
                    logger.error(f"Failed to revert state for {doc_index.document_id}:{doc_index.index_type}: {revert_error}")
    ...
    def _reconcile_document_operations(self, document_id: str, operations: dict):
        ...
        if create_index_types:
            ...
            logger.info(f"Scheduled create index task {task_id} for document {document_id} with types {create_index_types}")
        ...
        if update_index_types:
            ...
            logger.info(f"Scheduled update index task {task_id} for document {document_id} with types {update_index_types}")
        ...
        if delete_index_types:
            ...
            logger.info(f"Scheduled delete index task {task_id} for document {document_id} with types {delete_index_types}")
