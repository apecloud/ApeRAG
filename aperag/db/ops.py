...
class DatabaseOps:
    ...
    def create_document_indexes(self, document_id: str, index_types: List[str] = None) -> List:
        """
        Create index records for a document
        
        Args:
            document_id: Document ID
            index_types: List of index types to create. If None, creates all types.
        ...
        def _operation(session):
            ...
            for index_type in index_types:
                ...
                existing = session.query(DocumentIndex).filter_by(
    ...
    def query_pending_indexes(self, index_type: str = None, limit: int = 10):
        """
        Query pending index records
        ...
        def _operation(session):
            ...
            if index_type:
                query = query.filter(DocumentIndex.index_type == index_type)
            
            return query.order_by(DocumentIndex.gmt_created).limit(limit).all()
        
        return self._execute_transaction(_operation)
    
    def query_failed_retryable_indexes(self, limit: int = 10):
        """
        Query failed indexes that can be retried
        ...
        def _operation(session):
            indexes = session.query(DocumentIndex).filter(
                ...
                DocumentIndex.retry_count < DocumentIndex.max_retries
            ).order_by(DocumentIndex.gmt_modified).limit(limit).all()
            
            return [idx for idx in indexes if idx.can_retry()]
        
        return self._execute_transaction(_operation)
    
    def query_document_indexes(self, document_id: str):
        """
        Query all indexes for a document
    ...
    def update_document_index(self, index_id: int, **kwargs):
        """
        Update a document index record
        
        Args:
            index_id: DocumentIndex ID
    ...
    def mark_document_indexes_as_deleted(self, document_id: str):
        """
        Mark all indexes for a document as deleted
    ...
    def delete_document_indexes(self, document_id: str):
        """
        Delete all index records for a document
