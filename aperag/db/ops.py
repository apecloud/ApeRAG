...
async def build_pr(session: AsyncSession, model: Type[T], stmt, pq: PagedQuery) -> PagedResult:
    ...
    if not pq or not pq.page_number or not pq.page_size:
        ...
        return PagedResult(count=count, page_number=1, page_size=count, data=result.scalars().all())
    offset = (pq.page_number - 1) * pq.page_size
    result = await session.execute(stmt.offset(offset).limit(pq.page_size))
    return PagedResult(count=count, page_number=pq.page_number, page_size=pq.page_size, data=result.scalars().all())


async def query_collection(session: AsyncSession, user: str, collection_id: str):
...
async def query_collections(session: AsyncSession, users: List[str], pq: PagedQuery = None):
...
async def query_collections_count(session: AsyncSession, user: str, pq: PagedQuery = None):
...
async def query_collection_without_user(session: AsyncSession, collection_id: str):
    stmt = select(Collection).where(Collection.id == collection_id, Collection.status != CollectionStatus.DELETED)
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_document(session: AsyncSession, user: str, collection_id: str, document_id: str):
...
async def query_documents(session: AsyncSession, users: List[str], collection_id: str, pq: PagedQuery = None):
    filters = [
...
async def query_documents_count(session: AsyncSession, user: str, collection_id: str, pq: PagedQuery = None):
    filters = [
        Document.user == user,
        Document.collection_id == collection_id,
        Document.status != CollectionStatus.DELETED,
...
async def query_apikeys(session: AsyncSession, user: str, pq: PagedQuery = None):
...
async def query_apikey(session: AsyncSession, user: str, apikey_id: str):
    stmt = select(ApiKey).where(ApiKey.id == apikey_id, ApiKey.user == user, ApiKey.status != BotStatus.DELETED)
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_chat(session: AsyncSession, user: str, bot_id: str, chat_id: str):
...
async def query_chat_by_peer(session: AsyncSession, user: str, peer_type, peer_id: str):
    stmt = select(Chat).where(
...
async def query_chats(session: AsyncSession, user: str, bot_id: str, pq: PagedQuery = None):
...
async def query_bot(session: AsyncSession, user: str, bot_id: str):
    stmt = select(Bot).where(Bot.id == bot_id, Bot.user == user, Bot.status != BotStatus.DELETED)
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_bots(session: AsyncSession, users: List[str], pq: PagedQuery = None):
...
async def query_bots_count(session: AsyncSession, user: str, pq: PagedQuery = None):
...
async def query_config(session: AsyncSession, key):
...
async def query_user_quota(session: AsyncSession, user: str, key: str):
...
async def query_msp_list(session: AsyncSession, user: str):
...
async def query_msp_dict(session: AsyncSession, user: str):
    stmt = select(ModelServiceProvider).where(
        ModelServiceProvider.user == user, ModelServiceProvider.status != ModelServiceProviderStatus.DELETED
    )
    result = await session.execute(stmt)
...
async def query_msp(session: AsyncSession, user: str, provider: str, filterDeletion: bool = True):
...
async def query_user_by_username(session: AsyncSession, username: str):
    stmt = select(User).where(User.username == username)
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_user_by_email(session: AsyncSession, email: str):
    stmt = select(User).where(User.email == email)
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_user_exists(session: AsyncSession, username: str = None, email: str = None):
    stmt = select(User)
    if username:
...
async def create_user(session: AsyncSession, username: str, email: str, password: str, role: Role):
...
async def set_user_password(session: AsyncSession, user: User, password: str):
...
async def query_invitation_by_token(session: AsyncSession, token: str):
    stmt = select(Invitation).where(Invitation.token == token)
    result = await session.execute(stmt)
    return result.scalars().first()


async def create_invitation(session: AsyncSession, email: str, token: str, created_by: str, role: Role):
...
async def mark_invitation_used(session: AsyncSession, invitation: Invitation):
...
async def query_invitations(session: AsyncSession):
...
async def list_user_api_keys(session: AsyncSession, username: str) -> List[ApiKey]:
...
async def create_api_key(session: AsyncSession, user: str, description: Optional[str] = None) -> ApiKey:
...
async def delete_api_key(session: AsyncSession, username: str, key_id: str) -> bool:
...
async def get_api_key_by_id(session: AsyncSession, user: str, id: str) -> Optional[ApiKey]:
...
async def get_api_key_by_key(session: AsyncSession, key: str) -> Optional[ApiKey]:
...
async def query_chat_feedbacks(session: AsyncSession, user: str, chat_id: str, pq: PagedQuery = None):
...
async def query_message_feedback(session: AsyncSession, user: str, chat_id: str, message_id: str):
...
async def query_first_user_exists(session: AsyncSession):
    stmt = select(User).where(User.gmt_deleted is None)
    result = await session.execute(stmt)
    return result.scalars().first() is not None


async def query_running_sync_histories(session: AsyncSession, user: str, collection_id: str):
    stmt = select(CollectionSyncHistory).where(
        CollectionSyncHistory.user == user,
        CollectionSyncHistory.status == CollectionSyncStatus.RUNNING,
        CollectionSyncHistory.gmt_deleted is None,
        CollectionSyncHistory.collection_id == collection_id,
    )
    result = await session.execute(stmt)
    return result.scalars().all()


async def query_sync_history(session: AsyncSession, user: str, collection_id: str, sync_history_id: str):
    stmt = select(CollectionSyncHistory).where(
        CollectionSyncHistory.user == user,
        CollectionSyncHistory.collection_id == collection_id,
        CollectionSyncHistory.id == sync_history_id,
    )
    result = await session.execute(stmt)
    return result.scalars().first()


async def query_sync_histories(session: AsyncSession, user: str, collection_id: str, pq: PagedQuery = None):
    filters = [
        CollectionSyncHistory.user == user,
        CollectionSyncHistory.collection_id == collection_id,
        CollectionSyncHistory.gmt_deleted is None,
    ]
    filters += build_filters(CollectionSyncHistory, pq)
    stmt = select(CollectionSyncHistory).where(*filters).order_by(build_order_by(CollectionSyncHistory, pq))
    return await build_pr(session, CollectionSyncHistory, stmt, pq)
