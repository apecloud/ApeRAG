...
class JinaRerankRunner:
    async def rerank(self, invocation: ResolvedModelInvocation, query: str, documents: list[str], **kwargs):
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            data = response.json()
        return [
            {"index": item.get("index", idx), "relevance_score": item.get("relevance_score", item.get("score", 0.0))}
            for idx, item in enumerate(data.get("results", []))
        ]

    async def chat(self, invocation: ResolvedModelInvocation, messages: list[dict], **kwargs):
        raise NotImplementedError("Jina rerank runner only supports rerank")
