...
async def read_document_section(
    ...
    *,
    ...
    # 5. Fetch + slice authoritative markdown — D10.g cache-wrapped.
    # Tenancy/auth above are NEVER skipped (§E.7 hard lock).
    cache = await get_read_primitive_cache()

    async def _compute() -> DocumentSection:
    ...
    return await cache.get_or_compute_section(
        document_id=document.id,
        parse_version=parse_version,
        section_path=section_path,
        heading_anchor=heading_anchor,
        compute=_compute,
        model_cls=DocumentSection,
    )
