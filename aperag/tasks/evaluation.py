...
# Concurrency limit for running evaluations, configurable via environment variable
...
class EvaluationTask:
    ...
    async def _coordinate_evaluation(self, session: AsyncSession, evaluation: Evaluation):
        ...
        lock_name = f"evaluation_processing:{evaluation.id}"
        lock = get_or_create_lock(lock_name, lock_type="redis", expire_time=30)
    ...
    async def initialize_evaluation(self, evaluation_id: str):
        ...
        async for session in get_async_session():
            try:
                ...
                async for error_session in get_async_session():
    ...
    async def process_evaluation(self, evaluation_id: str):
        ...
        processing_lock = get_or_create_lock(lock_name, lock_type="redis", expire_time=120)
