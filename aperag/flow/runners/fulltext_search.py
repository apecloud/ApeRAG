...
class FulltextSearchNodeRunner(BaseNodeRunner):
    ...
    async def run(self, ui: FulltextSearchInput, si: SystemInput) -> Tuple[FulltextSearchOutput, dict]:
        ...
        if regular_collection_ids:
            regular_docs = await self.service.execute_fulltext_search(
                user=si.user,
                query=si.query,
                top_k=ui.top_k,
        ...
        if chat_id and chat_collection_id:
            chat_docs = await self.service.execute_fulltext_search(
                user=si.user,
                query=si.query,
                top_k=ui.top_k,
            ...
            all_docs.extend(chat_docs)

        return FulltextSearchOutput(docs=all_docs), {}
