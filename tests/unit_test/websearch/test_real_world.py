...
class TestRealWorldSearch:
    ...
    async def test_duckduckgo_real_search(self):
        ...
        try:
            ...
            response = await service.search(WebSearchRequest(
    ...
    async def test_site_specific_search(self):
        ...
        try:
            ...
            response = await service.search(WebSearchRequest(
                ...
                use_source_domain_only=True,
                max_results=3,
                timeout=15
            ))
            
            print("\n=== Site-specific Search: 'machine learning' on stackoverflow.com ===")
            print(f"Results: {len(response.results)}")
...
class TestRealWorldReading:
    ...
    async def test_batch_reading(self):
        ...
        try:
            ...
            response = await service.read(WebReadRequest(
                urls=urls,
                timeout=15,
                max_concurrent=2
            ))
