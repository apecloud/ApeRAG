...
_TASK10_LEGACY_DELETED = True  # flipped True in PR #1765 (W7-10 close-
# out fold-in of #1763). When True, every pattern below MUST return 0
# hits (modulo the documented exception paths).
...
# Path to this helper file itself, used to auto-exclude self-matches
# (every pattern is mentioned in our own docstring + ``_assert_zero``
# call site, which would otherwise self-trigger).
_SELF_REL = "tests/integration/test_w7_grep_zero_legacy_graphindex.py"
...
def _rg_count(
    ...
    *,
    ...
    # Always exclude self-matches: this helper file documents every
    # pattern in its own module docstring + assertion call site.
    effective_excludes = list(exclude_paths or []) + [_SELF_REL]
    kept = []
    for line in lines:
        if not any(exc in line for exc in effective_excludes):
            kept.append(line)
    lines = kept
...
def test_no_from_aperag_domains_knowledge_graph_graphindex_imports():
    ...
    _assert_zero(
        ...
        exclude_paths=[
            # Sister grep-zero check: asserts the legacy import string
            # is *absent* from a rendered prompt body, so the pattern
            # appears as a string literal in a ``not in`` assertion.
...
def test_no_legacy_fallback_truncate():
    ...
    Renamed-and-kept exception: the new ``GraphIndexCompactor``
    (Wave 7 W7-2) deliberately reuses ``_fallback_truncate`` for the
    graceful-degrade branch — same name, new home — so the new module
    + its tests are the documented allowed mentions."""
    _assert_zero(
        r"_fallback_truncate",
        exclude_paths=[
            "aperag/indexing/graph_compactor.py",
            "tests/unit_test/indexing/test_graph_compactor.py",
        ],
        note=(
...
def test_no_legacy_query_context_except_compose_context():
    ...
    ``GraphSearchService.compose_context``. Reality: at the
    indexing layer the new method is ``compose_context``; at the
    retrieval ``GraphSearchContract`` Protocol layer the method
    name was deliberately kept as ``query_context`` (it's a port,
    not a binding to the old class). The grep below targets bare
    ``query_context`` across the repo, with documented exceptions
    for the ports module + the few retrieval comments that name
    the legacy ``GraphIndexService.query_context`` for historical
    context."""
    _assert_zero(
        r"\bquery_context\b",
        exclude_paths=[
            # New ``GraphSearchContract.query_context`` Protocol port.
            "aperag/domains/retrieval/ports.py",
            # Pipeline doc-comments referencing the legacy entry +
            # the actual port call sites.
            "aperag/domains/retrieval/pipeline.py",
            # Indexing-layer comment naming the legacy entry as the
            # historical predecessor of ``compose_context``.
            "aperag/indexing/graph.py",
        ],
        note=(
            "Use GraphSearchService.compose_context (Wave 7 W7-5). "
            "Note: the indexing-layer method has a different name; "
            "the retrieval Protocol port keeps ``query_context`` "
            "intentionally (port semantics, not class binding)."
        ),
    )
...
def test_no_legacy_merge_entities_on_graphindex():
    ...
    Documented exceptions: docstrings + comments that reference the
    legacy fully-qualified name as historical context (i.e. "Replaces
    the legacy ..." / "not the legacy ...") are allowed — they
    document intent, they don't call the legacy code."""
    _assert_zero(
        r"GraphIndexService\.merge_entities",
        exclude_paths=[
            "aperag/graph_curation/lineage_merge.py",
            "tests/unit_test/indexing/test_wave7_task8_wiring.py",
        ],
