"""
Nebula-specific E2E tests using the universal graph storage test suite.
...
_nebula_test_storage_cache = {}


@pytest_asyncio.fixture(scope="function")
async def storage_with_data(mock_embedding_func, graph_data):
    """Initialize Nebula storage and populate it with test data, with caching"""
    global _nebula_test_storage_cache

    cache_key = "nebula_test_data"

    # Check if we already have initialized storage
    if cache_key in _nebula_test_storage_cache:
        storage, cached_graph_data = _nebula_test_storage_cache[cache_key]
        ...
        try:
            ...
            del _nebula_test_storage_cache[cache_key]

    # Initialize new storage
    import uuid

    workspace = f"test_nebula_persistent_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        for edge in graph_data["edges"]:
            try:
                ...
                if start_exists and end_exists:
                    await storage.upsert_edge(edge["start_node_id"], edge["end_node_id"], edge["properties"])
                    edge_count += 1
                    if edge_count % 500 == 0:
        ...
        _nebula_test_storage_cache[cache_key] = (storage, graph_data)
...
class TestNebulaStorage(GraphStorageTestRunner):
    """
    Nebula-specific test class that inherits all tests from GraphStorageTestRunner.

    This class automatically runs all BaseGraphStorage interface tests against
    the Nebula storage implementation.

    Note: Some tests may be skipped if Nebula Graph is not properly configured
    or if schema creation issues occur.
    """
